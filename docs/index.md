# Index de la documentation

Cet index regroupe les documents existants sans remplacer les fichiers sources.
Il distingue la vision produit, le pipeline réellement implémenté, la qualité et
l'évolution documentaire du dépôt.

## Vision et spécifications

- [PRD](PRD.md)
- [Spécification fonctionnelle](functional-spec.md)
- [Spécification des données](data-spec.md)

## Pipeline implémenté

- [Collecte Sudoc SRU](sru-collection.md)
- [Mapping UNIMARC](unimarc-mapping.md)
- [Audit local des extractions](quality-audit.md)
- [Normalisation Dewey et préparation de l'enrichissement BnF](dewey-enrichment.md)
- [Enrichissement BnF / IdRef et chargement DuckDB](enrichment-and-database.md)
- [Classifications IdRef via les seules têtes de vedettes 606$a](idref-subject-classifications.md)
- [Contrats documentaires des sorties](output-contracts.md)
- [Parcours de reproduction](reproduction.md)

## Qualité et tests

- [Plan de validation et de tests](validation-plan.md)
- [Synthèse des décisions documentées](decisions.md)

## Architecture et évolution

- [Architecture technique](architecture.md)
