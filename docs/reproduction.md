...
> Convention de version : `scripts/03_parse_unimarc.py` appelle le parseur inclus dans `src/analyseur_sudoc/unimarc.py`. Les dossiers `unimarc-v0.3.7` et `unimarc-v0.3.8` correspondent à la version interne `PARSER_VERSION` du parseur, pas à la version du paquet Python déclarée dans `pyproject.toml`.
