...
def collect_sudoc(client: httpx.Client, run_dir: Path, *, year=2025, page_size=100,
                  max_pages=None, delay=0.5, sleep=time.sleep,
    ...
    Une nouvelle exécution sans cette limite poursuit donc un essai interrompu.
    """
    ...
    if manifest_path.exists():
        ...
        if any(manifest.get(key) != value for key, value in settings.items()):
            raise ValueError("Paramètres différents de la campagne existante (année, taille de page ou version)")
    else:
        ...
        for prefix, query in queries.items():
            ...
            while True:
                ...
                if path.exists() and metadata_path.exists():
                    ...
                    if metadata.get("params") != request_params(query, start, page_size):
                        raise SruError(f"Paramètres du cache incohérents : {path}")
                    page = parse_page(raw, start, page_size, query)
                else:
                    raw, page = download_page(client, run_dir, prefix, query, start, page_size,
                                              delay=delay, sleep=sleep)
                    metadata = {"retrieved_at": now(), "params": request_params(query, start, page_size),
